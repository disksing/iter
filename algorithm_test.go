...
var (
	ibegin  = SliceBegin[int]
	irbegin = SliceRBegin[int]
	iend    = SliceEnd[int]
	irend   = SliceREnd[int]

	bbegin  = SliceBegin[byte]
	brbegin = SliceRBegin[byte]
	bend    = SliceEnd[byte]
	brend   = SliceREnd[byte]
)
...
func randString() string {
	...
	Iota(bbegin(&alphabets), bend(&alphabets), byte('a'))
...
func TestAllAnyNoneOf(t *testing.T) {
	...
	s := randIntSlice()
	assert.Equal(AllOf(ibegin(&s), iend(&s), pred), allOf(s))
	assert.Equal(AnyOf(ibegin(&s), iend(&s), pred), anyOf(s))
	assert.Equal(NoneOf(ibegin(&s), iend(&s), pred), noneOf(s))
}
